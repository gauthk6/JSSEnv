...
"""
Improved Multi-Instance PPO Training for JSSP with Performance Enhancements
- Reward shaping with heuristic guidance
- Curriculum learning (easy->hard instances)
- Better exploration strategies
- Makespan-aware rewards
- Machine utilization rewards
"""
...
def parse_args():
    ...
    parser.add_argument("--total-timesteps", type=int, default=8000000,
        help="total timesteps of the experiments (extended for better learning)")
    ...
    parser.add_argument("--gamma", type=float, default=0.995,
        help="the discount factor gamma (higher for long-term planning)")
    ...
    parser.add_argument("--ent-coef", type=float, default=0.02,
        help="coefficient of the entropy (higher for better exploration)")
    ...
    parser.add_argument("--reward-shaping", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="Enable reward shaping with heuristic guidance")
    parser.add_argument("--makespan-weight", type=float, default=0.1,
        help="Weight for makespan improvement in reward shaping")
    parser.add_argument("--utilization-weight", type=float, default=0.05,
        help="Weight for machine utilization in reward shaping")
    parser.add_argument("--curriculum-learning", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="Enable curriculum learning (start with easier instances)")
    parser.add_argument("--curriculum-phases", type=int, default=4,
        help="Number of curriculum phases")
    parser.add_argument("--exploration-bonus", type=float, default=0.01,
        help="Bonus for exploring less-used actions")
    parser.add_argument("--heuristic-guidance", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="Use heuristic guidance in reward shaping")
...
class RewardShaper:
    """Advanced reward shaping for JSSP using heuristic guidance"""
    
    def __init__(self, makespan_weight=0.1, utilization_weight=0.05, 
                 exploration_bonus=0.01, heuristic_guidance=True):
        self.makespan_weight = makespan_weight
        self.utilization_weight = utilization_weight
        self.exploration_bonus = exploration_bonus
        self.heuristic_guidance = heuristic_guidance
        self.prev_makespan = None
        self.action_counts = {}
        
    def shape_reward(self, base_reward, env_state, action_taken, env):
        ...
        # 1. Makespan progress reward
        if self.makespan_weight > 0:
            current_makespan = actual_env.current_time_step
            if self.prev_makespan is not None:
                makespan_improvement = max(0, self.prev_makespan - current_makespan)
                shaped_reward += self.makespan_weight * makespan_improvement
            self.prev_makespan = current_makespan
        
        # 2. Machine utilization reward
        if self.utilization_weight > 0:
            busy_machines = np.sum(actual_env.time_until_available_machine > 0)
            utilization_rate = busy_machines / actual_env.machines
            shaped_reward += self.utilization_weight * utilization_rate
        
        # 3. Exploration bonus
        if self.exploration_bonus > 0:
            action_key = f"{action_taken}"
            self.action_counts[action_key] = self.action_counts.get(action_key, 0) + 1
            exploration_reward = self.exploration_bonus / (1 + self.action_counts[action_key])
            shaped_reward += exploration_reward
        
        # 4. Heuristic guidance reward
        if self.heuristic_guidance:
            heuristic_reward = self._get_heuristic_guidance_reward(env_state, action_taken, actual_env)
            shaped_reward += 0.02 * heuristic_reward  # Small weight for guidance
        
        return shaped_reward
    
    def _get_heuristic_guidance_reward(self, obs_dict, action_taken, actual_env):
        """Reward for taking actions similar to good heuristics"""
        if action_taken == actual_env.NO_OP_ACTION:
            return 0.0
        ...
        # SPT preference: reward choosing jobs with shorter processing times
        job_idx = action_taken
        ...
        if current_op_idx < actual_env.machines:
            ...
            # Calculate relative ranking (lower processing time = higher reward)
            other_op_times = []
            for other_action in valid_actions:
                if other_action != actual_env.NO_OP_ACTION and other_action != action_taken:
                    other_job = other_action
                    other_op_idx = actual_env.todo_time_step_job[other_job]
                    if other_op_idx < actual_env.machines:
                        other_time = actual_env.instance_matrix[other_job, other_op_idx]['time']
                        other_op_times.append(other_time)
            
            if other_op_times:
                # Reward inverse to processing time rank
                rank = sum(1 for t in other_op_times if current_op_time <= t)
                return rank / len(other_op_times)
...
class CurriculumManager:
    ...
    def __init__(self, train_instances, phases=4, phase_length=2000000):
        self.train_instances = train_instances
        self.phases = phases
        self.phase_length = phase_length
        self.current_phase = 0
        
        # Sort instances by estimated difficulty (based on makespan heuristics)
        # ta01, ta02 are typically easier than ta04, ta05
        instance_difficulty = {
            "ta01": 1, "ta02": 2, "ta03": 3, "ta04": 4, "ta05": 5,
            "ta06": 6, "ta07": 7, "ta08": 8, "ta09": 9, "ta10": 10
        }
        
        self.sorted_instances = sorted(
            [inst for inst in train_instances if inst in instance_difficulty],
            key=lambda x: instance_difficulty[x]
        )
        
        print(f"Curriculum order: {self.sorted_instances}")
    
    def get_current_instances(self, global_step):
        """Get instances for current training phase"""
        new_phase = min(global_step // self.phase_length, self.phases - 1)
        
        if new_phase != self.current_phase:
            self.current_phase = new_phase
            print(f"=== CURRICULUM PHASE {new_phase + 1}/{self.phases} at step {global_step} ===")
        
        # Gradually add more instances
        if self.current_phase == 0:
            # Phase 1: Easiest instance only
            return self.sorted_instances[:1]
        elif self.current_phase == 1:
            # Phase 2: 2 easiest instances
            return self.sorted_instances[:2]
        elif self.current_phase == 2:
            # Phase 3: 3 easiest instances
            return self.sorted_instances[:3]
        else:
            # Phase 4+: All instances
            return self.sorted_instances

def make_env_improved(env_id, seed, idx, capture_video, run_name, curriculum_manager, reward_shaper):
    def thunk():
        project_root = os.path.dirname(os.path.abspath(__file__))
        
        class ImprovedMultiInstanceWrapper(gym.Wrapper):
            def __init__(self, env_id, curriculum_manager, reward_shaper, project_root):
                self.env_id = env_id
                ...
                # Initialize with first curriculum instance
                self._create_env()
                super().__init__(self.env)
                
            def _create_env(self):
                # Get current curriculum instances
                current_instances = self.curriculum_manager.get_current_instances(self.global_step)
                
                # Randomly select from current curriculum
                instance_name = random.choice(current_instances)
            ...
            def reset(self, **kwargs):
                # Update global step for curriculum
                self.global_step = getattr(self, '_global_step_counter', 0)
                
                # Create new environment with curriculum-appropriate instance
                self._create_env()
                obs, info = self.env.reset(**kwargs)
                
                # Reset reward shaper state
                self.reward_shaper.prev_makespan = None
            ...
            def step(self, action):
                ...
                if self.reward_shaper:
                    shaped_reward = self.reward_shaper.shape_reward(reward, obs, action, self.env)
                else:
            ...
            def set_global_step(self, step):
                self._global_step_counter = step
        
        # Create the improved wrapper
        env = ImprovedMultiInstanceWrapper(env_id, curriculum_manager, reward_shaper, project_root)
...
class ImprovedAgent(nn.Module):
    """Enhanced agent with better architecture for JSSP"""
    ...
    def __init__(self, envs):
        ...
        # Deeper networks for better representation learning
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id.split('/')[-1]}__IMPROVED__{args.exp_name}__{timestamp}"
    ...
    print(f"🚀 IMPROVED PPO Training with Advanced Features")
    ...
    print(f"Heuristic guidance: {args.heuristic_guidance}")
    
    # Initialize improvement components
    curriculum_manager = CurriculumManager(
        args.train_instances, 
        phases=args.curriculum_phases,
        phase_length=args.total_timesteps // args.curriculum_phases
    ) if args.curriculum_learning else None
    ...
    ) if args.reward_shaping else None
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env_improved(args.env_id, args.seed + i, i, False, run_name, curriculum_manager, reward_shaper) for i in range(args.num_envs)]
    )
    ...
    agent = ImprovedAgent(envs).to(device)
    ...
    print(f"\n🎯 Starting IMPROVED training for {args.total_timesteps} timesteps ({num_updates} PPO updates).")
    ...
    print(f"  Entropy coefficient: {args.ent_coef} (enhanced exploration)")
    print(f"  Gamma: {args.gamma} (better long-term planning)")
    ...
    for update in range(1, num_updates + 1):
        ...
        for step_idx in range(0, args.num_steps):
            ...
            # Episode completion logging (same as before)
        ...
        if update % 20 == 0:
            ...
            if curriculum_manager:
                current_instances = curriculum_manager.get_current_instances(global_step)
                print(f"  📚 Curriculum Phase {curriculum_manager.current_phase + 1}: {current_instances}")
        ...
        if writer:
            ...
            # Curriculum tracking
            if curriculum_manager:
                writer.add_scalar("curriculum/phase", curriculum_manager.current_phase, global_step)
                writer.add_scalar("curriculum/num_instances", len(curriculum_manager.get_current_instances(global_step)), global_step)
    ...
    # Save final model
    final_model_path = f"runs/{run_name}/improved_agent_{timestamp}.pt"
    os.makedirs(os.path.dirname(final_model_path), exist_ok=True)
    torch.save(agent.state_dict(), final_model_path)
    print(f"🎉 IMPROVED training finished!")
    print(f"📁 Final improved agent model saved to {final_model_path}")
